...
/* Cgcx seed token sale contract, where buyers send ethers to receive ERC-20
 * CGX tokens in return. It features:
 * - instant token payback when eth is sent
 * - max funding and max CGX tokens hard-caps
 ...
 *
 * Amount of CGX tokens send back to buyers decreases linearly: early buyers
 ...
 */
contract CgcxSeedSale is DSAuth, DSMath {
    ...
    DSToken public cgxToken;         // CGX token contract
    ...
    function CgcxSeedSale(DSToken cgxToken_, address beneficiary_) {
        cgxToken = cgxToken_;
    ...
    function buyTokens() saleIn(State.Running) inRunningBlock ethSent payable {
        ...
        cgxToken.mint(msg.sender, tokensBought);
