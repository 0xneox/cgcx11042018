...
def test_init(chain, token, beneficiary):
    sale = deploy_contract(chain, 'CgcxSeedSale', args=[token.address, beneficiary])

    assert sale.call().cgxToken() == token.address
